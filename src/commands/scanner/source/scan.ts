...
export default class Scan extends SfdxCommand {
  ...
  protected static flagsConfig = {
    ...
    rulesetfile: flags.string({ char: 's', description: messages.getMessage('ruleSetFileDescription'), required: true }),
    targetdir: flags.string({ char: 'd', description: messages.getMessage('targetDirectoryDescription'), default: 'force-app' }),
  ...
  public async run(): Promise<FileAlert[]> {
    ...
    if (errorLevel < 1 || errorLevel > 5) {
      throw new SfdxError('The provided error level sits outside the severity range, enter a number between 1 and 5');
    }
